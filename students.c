...
int find_students(const char *student_last_name) {
    ...
    while (!feof(students_file)) {
        fscanf(students_file, "%s\n", buff);
        number = strtok(buff, ";");
        last_name = strtok(NULL, ";");
        first_name = strtok(NULL, ";");
        patronymic = strtok(NULL, ";");
        faculty = strtok(NULL, ";");
        specialty = strtok(NULL, ";");

        if (!strcmp(student_last_name, last_name)) {
...
int has_student(const char *number) {
    ...
    while (!feof(students_file)) {
        ...
        db_number = strtok(buff, ";");

        if (!strcmp(number, db_number)) {
            fclose(students_file);
            return SUCCESS;
        }
    }
...
int remove_student(const char *number) {
    ...
    while (!feof(students_file)) {
        ...
        if (strncmp(number, buff, strlen(number)) != 0) {
